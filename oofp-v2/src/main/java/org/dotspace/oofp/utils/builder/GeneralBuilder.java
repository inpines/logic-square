...
public class GeneralBuilder<P, T> {
	...
	private final List<GeneralBuildingWriter<T>> writers = new ArrayList<>();
	...
	public GeneralBuilder<P, T> with(GeneralBuildingWriter<T> writer) {
		writers.add(writer);
		return this;
	}
	
	public GeneralBuilder<P, T> with(
			Collection<GeneralBuildingWriter<T>> writer) {
