...
public interface Validation<E extends Joinable<E>, T> {
    ...
    static <T, M extends MessageCodeItem> Validation<Violations, T> require(
            T value, M message, Object... args) {
        return require(value, () -> Pair.of(message, args));
    }

    static <T, M extends MessageCodeItem> Validation<Violations, T> require(
            T value, Supplier<Pair<M, Object[]>> errorMessageSupplier) {
        Pair<M, Object[]> errorMessage = errorMessageSupplier.get();
        M message = errorMessage.getLeft();
        Object[] messageArgs = errorMessage.getRight();
        return value != null ? Validation.valid(value) : Validation.invalid(
                Violations.violate(message.getCode(), message.formatFrom(messageArgs))
        );
    }
