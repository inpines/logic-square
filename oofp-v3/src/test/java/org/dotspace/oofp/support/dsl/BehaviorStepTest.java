...
class BehaviorStepTest {
    ...
    void testOf() {
        BehaviorStep<Integer> step = Validation::valid;
        var out = step.execute(ctx(5));
    ...
    void testChain_successFlow() {
        ...
        var out = chained.execute(ctx(3)); // (3 + 1) * 2 = 8
    ...
    void testChain_shortCircuitOnInvalid() {
        ...
        when(neverCalled.execute(any())).thenAnswer(inv -> Validation.valid(inv.getArgument(0)));

        BehaviorStep<Integer> chained = BehaviorStep.chain(List.of(invalidFirst, neverCalled));

        var out = chained.execute(ctx(10));
    ...
    void testWhen_conditionallyRun() {
        ...
        assertEquals(2, payloadOf(guarded.execute(ctx(1))));
        assertEquals(0, payloadOf(guarded.execute(ctx(0)))); // 未執行 inc
    }
    ...
    void testAndThen() {
        ...
        assertEquals(8, payloadOf(step.execute(ctx(5))));
    }
    ...
    void testAndThenMapper() {
        ...
        assertEquals(50, payloadOf(step.execute(ctx(5))));
    }
    ...
    void testFilter() {
        ...
        var ok = step.execute(ctx(10));
        ok.fold(err -> fail("應為 Valid"), okv -> okv);

        var bad = step.execute(ctx(3));
    ...
    void testPeek() {
        ...
        fail.peek(sc -> called.set(true)).execute(ctx(2));
    ...
    void testPeekOnError() {
        ...
        BehaviorStep<Integer> fail = in -> Validation.invalid(in.withViolation(Violations.empty()));
        fail.peekOnError(v -> handled.set(true)).execute(ctx(0));
    ...
    void testRecover_successfulRecovery() {
        ...
        var out = recovered.execute(ctx(7));
